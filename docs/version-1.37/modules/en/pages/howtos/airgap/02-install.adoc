...
:description: Configure a private registry with {admc-name} images and policies to install {admc-name} in an air-gapped environment.
...
For an air-gapped installation of {admc-name}, you need a private Open
Container Initiative (OCI) registry accessible by your Kubernetes cluster.
{admc-short-name} Policies are WebAssembly modules, therefore you can store
them in an OCI-compliant registry as OCI artifacts. You need to add
{admc-short-name}’s images and policies to this OCI registry. You can refer
to a list of {admc-short-name} artifacts in the
xref:/reference/artifacts.adoc[Artifacts reference] page. The following
sections describe the process.

[NOTE]
====
We recommend using link:https://docs.hauler.dev/docs/intro[Hauler].
See our xref:/howtos/airgap/03-hauler.adoc[Hauler documentation page].
...
xref:/tutorials/verifying-kubewarden.adoc#_helm_chart[helm charts] and
xref:/tutorials/verifying-kubewarden.adoc#_container_images[container images].
====

== Save container images locally

. Each of our Helm charts contains an `imagelist.txt` with the container
...
. Save {admc-short-name} container images into a `.tar.gz` file:
+
[,shell]
----
...
----
+
Docker begins pulling the images used for an air gap install. This
process can take a few minutes. When complete, your current directory, where you
...
----
+
A file with a list of default policies is in the {admc-short-name} defaults
...
. Download `kubewarden-save-policies.sh` and `kubewarden-load-policies.sh` from
  the https://github.com/kubewarden/adm-controller/blob/main/scripts[`adm-controller` repository].
...
* Helm charts in `tgz` format (for example, `kubewarden-crds-1.23.0.tgz`)
...
. Load {admc-short-name} images into the private registry. You need to authenticate
...
. Load {admc-short-name} policies into the private registry. You should authenticate
...
Now you can create {admc-short-name} policies in your cluster. Policies must be
