...
:title: kwctl CLI
:description: 探索 \`kwctl` 命令行工具以管理 {admc-name} 策略并执行加载策略等任务。
...
* `annotate` — 向 WebAssembly 模块添加 {admc-short-name} 元数据
* `bench` — 对 {admc-short-name} 策略进行基准测试
* `completions` — 生成 shell 补全
* `digest` — 从策略的 OCI 清单中获取摘要
* `docs` — 为 kwctl 命令生成 markdown 文档
* `info` — 显示系统信息
* `inspect` — 检查 {admc-short-name} 策略
* `load` — 从 tar.gz 文件加载策略
* `policies` — 列出所有已下载的策略
* `pull` — 从给定的 URI 拉取 {admc-short-name} 策略
* `push` — 将 {admc-short-name} 策略推送到 OCI 注册表
* `rm` — 从存储中移除 {admc-short-name} 策略
* `run` — 从给定的 URI 运行 {admc-short-name} 策略
* `save` — 将策略保存到 tar.gz 文件
* `scaffold` — 为 Kubernetes 资源或配置文件生成骨架
* `verify` — 使用 Sigstore 验证来自给定 URI 的 {admc-short-name} 策略
...
向 WebAssembly 模块添加 {admc-short-name} 元数据
...
- 本地 YAML 文件：例如，`file://home/tux/cluster-admission-policy.yaml`（包含 {admc-short-name} 自定义资源的声明，如 `ClusterAdmissionPolicy`、`AdmissionPolicy` 等）

默认行为:
如果省略模式，则假定为 `file://`，根目录为当前目录。

关于 {admc-short-name} 自定义资源的说明： +
- 标志 `--request-path`、`--settings-path` 和 `--settings-json` 被忽略；设置从自定义资源定义中读取。 +
- `--execution-mode` 标志适用于 YAML 文件中的所有策略。 +
- `--raw` 标志不能使用，因为 {admc-short-name} 的自定义资源不支持 `raw` 策略。
...
* `<URI_OR_SHA_PREFIX_OR_YAML_FILE>` — 策略 URI、SHA 前缀或包含 {admc-short-name} 策略资源的 YAML 文件。支持的方案：registry://、link:https://[], file://。如果省略模式，则假定为 file://，根目录为当前目录。
...
检查 {admc-short-name} 策略

*用法：* `kwctl inspect [OPTIONS] <uri_or_sha_prefix>`

==== 参数

* `<URI_OR_SHA_PREFIX>` — 策略 URI 或 SHA 前缀。支持的方案：registry://、link:https://[], file://。如果省略模式，则假定为 file://，根目录为当前目录。
...
从给定 URI 拉取 {admc-short-name} 策略

*用法：* `kwctl pull [OPTIONS] <uri>`

==== 参数

* `<URI>` — 策略 URI。支持的方案：registry://、link:https://[], file://
...
* `--github-owner <VALUE>` — 在 CD 管道中生成的证书中预期的 GitHub 拥有者。
* `--github-repo <VALUE>` — 在 CD 管道中生成的证书中预期的 GitHub 仓库。
* `-o`, `--output-path <PATH>` — 输出文件。如果未提供，将下载到 {admc-short-name} 存储
...
* `vap` — 将 Kubernetes `ValidatingAdmissionPolicy` 转换为 {admc-short-name} `ClusterAdmissionPolicy`
...
* `-o`, `--operation <TYPE>` — {admc-short-name} 自定义资源类型 +
  可能的值：`CREATE`
...
* `--sources-path <PATH>` — 包含源信息的YAML文件（https，不安全的注册表主机，自定义CA...）。
* `--title <VALUE>` — 策略标题
* `-t`, `--type <VALUE>` — {admc-short-name} 自定义资源类型 +
...
将 Kubernetes `ValidatingAdmissionPolicy` 转换为 {admc-short-name} `ClusterAdmissionPolicy`
...
使用 Sigstore 从给定的 URI 验证 {admc-short-name} 策略。
