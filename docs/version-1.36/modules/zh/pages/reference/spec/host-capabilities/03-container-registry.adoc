...
从知名的容器镜像到通用的OCI工件。
OCI工件用于存储对象，例如{admc-name}策略、Helm图表等。

这些是{admc-short-name}主机暴露的功能，用于与容器注册表交互。
...
计算摘要涉及{admc-short-name}策略主机与远程注册表之间的一系列网络请求。
这些操作可能耗时，因此结果会缓存1分钟。

=== 身份验证

与私有注册表的交互要求{admc-short-name}策略主机对远程注册表进行身份验证。
...
获取此信息涉及{admc-short-name}策略主机和远程注册表之间的网络请求。这些操作可能耗时，因此结果会缓存1分钟。
...
获取此信息涉及在{admc-short-name}策略主机和远程注册表之间的网络请求。这些操作可能耗时，因此结果会缓存1分钟。
