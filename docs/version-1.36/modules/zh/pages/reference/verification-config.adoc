...
* `githubAction`：对于在 GitHub Actions 中使用 Sigstore 的无密钥工作流进行的签名。{admc-short-name} 将此信息与由 GitHub 的 OpenID Connect 创建的 x509 证书扩展 `workflow_repository` 进行检查，而不仅仅是 `issuer` 和 `subject`。如果处理 GitHub Actions，您应使用此 `kind`。
...
使用{admc-short-name}，您可以验证策略签名是否来自受信任的用户*和*并具有特定的注释。
...
最后这个例子测试给定策略是否来自{admc-short-name}组织：
