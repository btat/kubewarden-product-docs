...
作为Kubernetes集群操作符，您需要对想要使用的{admc-name}策略进行测试。
...
{admc-short-name}有一个工具，link:https://github.com/kubewarden/kwctl[`kwctl`]，允许在Kubernetes外部测试策略。
...
正如link:https://github.com/kubewarden/psp-apparmor#configuration[策略文档]所述，这会导致禁止使用非默认控制文件。
...
您可以使用link:https://github.com/bats-core/bats-core[bats]自动化所有这些步骤。
