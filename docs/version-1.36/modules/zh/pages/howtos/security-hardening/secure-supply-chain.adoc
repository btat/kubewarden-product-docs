...
:description: 配置 {admc-name} 以验证工件签名和接纳政策的完整性。
...
:doc-type: ["howto"]
:doctype: book
:keywords: ["{admc-name}", "kubewarden", "kubernetes", "安全供应链", "基础设施"]
:sidebar_label: 安全供应链
:sidebar_position: 40
...
{admc-short-name} 使用 https://github.com/sigstore/cosign[`cosign`] 以及 https://github.com/SigStore/fulcio[fulcio] 和 https://github.com/sigstore/rekor[rekor] 基础设施，这些都是由 Sigstore 项目提供的。

集群操作员可以配置 {admc-name} 仅运行由受信任实体签署的策略。策略开发者可以签署他们的策略并将其发布在注册表中。
...
{admc-short-name} 建议使用 Sigstore 的 https://github.com/sigstore/cosign[cosign] 工具来签署策略。本节展示了一种基于密钥的签署策略的方法。您需要为此生成一对私钥和公钥。生成的密钥有助于验证签署的工件是否来自预期的用户。要生成此密钥对，请使用此 `cosign generate-key-pair` 命令：
...
此命令通过创建一个新的签名对象来签署策略。签名对象随后与策略一起上传到注册表。现在该策略已准备好在使用签名验证的 {admc-short-name} 安装中使用。
...
在使用无密钥签名时，在 GitHub 操作中，`cosign` 不需要用户登录到 OIDC 提供者。在执行 GitHub 工作流程期间，可以使用 GitHub 令牌。它用于验证用户并生成临时密钥。签名过程与无密钥模式下使用的相同。这是 {admc-short-name} 项目如何签名其策略的示例：

.描述 {admc-short-name} 策略签名的 YAML
[%collapsible]
======
...
策略开发人员可以使用 {admc-short-name} 策略模板。他们有 GitHub 操作来构建、测试、签名和发布策略。
...
您可以使用 {admc-short-name} 和 `ConfigMap` 配置仅运行受信任的策略。`ConfigMap` 结构在 xref:reference/verification-config.adoc#_signature_configuration_reference[签名配置参考] 中描述。它用于使用 `kwctl` 验证策略。`ConfigMap` 应该在 `verification-config` 字段下定义允许的配置。

例如，您想运行由 {admc-short-name} GitHub 组织签名的策略。那么这个场景的示例 `ConfigMap` 将是：
...
使用 {admc-short-name}，您可以验证策略签名是否由受信任的用户签署，以及是否包含特定的注释。
...
最后一个示例测试给定策略是否来自 {admc-short-name} 组织：
