...
:keywords: "{admc-name}", Kubernetes, Kubewarden, Rancher Backup Operator, 备份, 恢复
...
自版本 `v9.0.0` 起，`rancher-backup` 支持 {admc-name}。这包括：

- 默认的 Rancher 名称空间 `cattle-kubewarden-system`（或 `cattle-kubewarden-*`），以及默认的 {admc-short-name} 名称空间 `kubewarden`。
- 所需的 {admc-short-name} 资源通过 Helm 图表安装。
- {admc-short-name} CRD，在恢复后由 {admc-short-name} 控制器进行协调。
...
请访问链接：link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/rancher-admin/back-up-restore-and-disaster-recovery/back-up-restore-and-disaster-recovery.html[] [Rancher Manager] 文档。对于使用 Minikube 安装的类型为 `hostPath` 的 PersistentVolumes，名称为 `standard`，Minikube 开箱即用支持。

需要指示 Backup Operator 通过将值 `optionalResources.kubewarden.enabled` 设置为 `true`，还备份 {admc-short-name} 用户定义的 CR。
...
使用`rancher-resource-set-full`备份{admc-short-name}机密。
