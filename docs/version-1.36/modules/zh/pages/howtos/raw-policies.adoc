...
:description: 将原始策略配置为通用策略评估引擎，使用 {admc-name}。
...
从 `v1.9.0` 开始，{admc-name} 支持在 Kubernetes 集群外部编写和执行策略的能力，作为通用策略评估引擎。
策略服务器公开了 `/validate_raw` 端点，可用于验证任意 JSON 文档与 {admc-short-name} 策略的匹配。
...
== 在 Kubernetes 中运行策略服务器而不使用 {admc-short-name} 控制器

无法使用由 {admc-short-name} 控制器管理的策略服务器实例来托管原始策略。
...
与验证 Kubernetes 资源的策略类似，原始策略是使用 {admc-short-name} SDK 在 WebAssembly 中编写的。
