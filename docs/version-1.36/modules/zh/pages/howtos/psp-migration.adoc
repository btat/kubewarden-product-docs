...
:description: 在您的 Kubernetes 集群中配置 {admc-name} 策略，以实现对策略配置的细粒度控制，替代 PodSecurityPolicy (PSP)。
...
https://kubernetes.io/docs/concepts/security/pod-security-policy/[PodSecurityPolicy]（PSP）已被移除。现在您可以在您的 Kubernetes 集群中使用 {admc-name} 进行准入控制。

{admc-short-name} 拥有单独的策略，以实现与单体 PSP 配置相同的目标。每个 {admc-short-name} 策略定义在 PSP 的规范中作为不同的配置部分。PSP 配置字段与其各自的 {admc-short-name} 策略的映射关系见下方 <<_mapping_policies_to_psp_fields,映射表>>。

使用 {admc-short-name}，操作员可以在其集群中对策略配置进行细粒度控制。

使用 {admc-short-name} 实例，您可以部署策略以替代 `PodSecurityPolicy` 对象。我们在这个例子中考虑这些规则：
...
== {admc-short-name} 替换 PSP

现在我们将创建 {admc-short-name} 策略以实现相同的目标。
您通过单独的 {admc-short-name} 策略来强制执行每条规则。
...
您应该使用`mutation: true`配置策略。这是必需的，因为当用户未定义它们时，策略将添加link:https://kubernetes.io/docs/concepts/security/pod-security-policy/#users-and-groups[supplementalGroups]。
...
下面的示例显示了添加一个link:https://kubernetes.io/docs/concepts/security/pod-security-policy/#users-and-groups[supplemental group]，尽管它并未被我们定义。
...
您需要替换阻止特权容器的旧PSP配置。部署link:https://github.com/kubewarden/pod-privileged-policy[pod-privileged policy]是必要的。此策略不需要任何设置。一旦运行，它将阻止特权 Pod。
...
为了完成 PSP 迁移练习，您需要禁用主机名称空间共享。为此，我们将使用link:https://github.com/kubewarden/host-namespaces-psp-policy[`host-namespace-psp` policy]。它允许集群管理员单独阻止 IPC、PID 和网络名称空间。它还设置了 Pod 可以在主机 IP 上开放的端口。
...
== 将 {admc-short-name} 策略映射到 PSP 字段

此表将 PSP 配置字段映射到相应的 {admc-short-name} 策略。

|===
| PSP 字段 | {admc-short-name} 等效策略
...
{admc-short-name}团队开发了一个用于PSP迁移的脚本。它使用来自link:https://github.com/appvia/psp-migration[AppVia]的迁移工具。AppVia 工具读取 PSP YAML 配置。它会生成相应的策略。它为 {admc-short-name} 和其他策略引擎执行此操作。

[CAUTION]
====

AppVia 迁移工具不在 Kubewarden 维护者的控制之下。
这意味着它可能会生成过时的 {admc-short-name} 策略。请谨慎使用。我们需要一个AppVia的拉取请求，目前正在进行中。
如果需要更多信息，请与我们联系。
====


该脚本可在{admc-short-name} https://github.com/kubewarden/utils/blob/main/scripts/psp-to-kubewarden[utils]储存库中获得。它将 AppVia 迁移工具下载到工作目录中以供使用。它处理在 `kubectl` 默认上下文中定义的 PSP。然后它在标准输出上打印 Kubewarden 策略定义。用户可以将内容重定向到文件或直接到`kubectl`。
...
{admc-short-name} 策略可以使用以下命令直接应用于集群：
