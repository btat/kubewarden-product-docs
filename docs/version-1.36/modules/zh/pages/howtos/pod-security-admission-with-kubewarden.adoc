...
= 使用 Pod 安全准入与 {admc-name}
:page-languages: [en, de, es, fr, ja, pt, zh]
:revdate: 2026-05-04
:page-revdate: {revdate}
:description: 在 Kubernetes 集群中配置 Pod 安全准入 {admc-name}，以保护 Pods。
...
:doc-type: ["howto"]
:keywords: ["{admc-name}", "kubewarden", "pod 安全准入", "pod 安全策略", "kubernetes"]
...
{admc-short-name} 可用于 *集成* PSA 控制文件，以避免这些限制。

[NOTE]
====

您可以使用{admc-short-name}来替换旧的 Pod 安全策略，如xref:howtos/psp-migration.adoc[PSP 迁移]所示。
然而，本文的目标是展示{admc-short-name}如何补充新的PSA。
====
...
在这里，{admc-short-name}可以提供帮助。
您可以使用link:https://artifacthub.io/packages/kubewarden/user-group-psp/user-group-psp[`user-group-policy`]策略来变更部署定义。
这为省略该信息的容器配置了默认用户。

[NOTE]
====

您需要在 Kubernetes 集群中使用 {admc-short-name} 堆栈来进行此示例。
...
现在部署受到{admc-short-name}的策略影响，该策略允许启动 Pod。
...
在这种情况下，{admc-short-name} 集成可以做更多。
它可以检查提供的 `runAsUser` 的值。

此资源被之前的 {admc-short-name} 策略拒绝：
...
将 {admc-short-name} 与 PSA 一起使用有助于填补这一空白。
