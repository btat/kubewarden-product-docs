...
:description: 配置 PolicyServers 使用私有或自托管的 Sigstore 实例进行 {admc-name} 的策略签名验证。
...
确保安全地从您的 Sigstore 实例下载证书和密钥。否则，您可能会使用被破坏的数据配置 {admc-name}。
...
[NOTE]
====
ConfigMap 键 **必须** 为 `verification-config`（而不是文件名）。{admc-short-name} 查找这个确切的键。
====
