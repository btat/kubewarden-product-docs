...
跟踪让您收集有关策略评估的详细信息。它可以帮助您调试 {admc-name} 部署和策略中的问题。
...
在撰写本文时（2022-06-21），只有特定版本的 Jaeger 与 Cert Manager 兼容，link:https://github.com/jaegertracing/helm-charts/blob/main/charts/jaeger-operator/COMPATIBILITY.md[请参阅兼容性图表]。
...
现在您可以按照通常的方式继续部署 {admc-short-name}。
...
作为第一步，您添加包含 {admc-short-name} 的 Helm 储存库：
...
然后您安装由 {admc-short-name} 定义的自定义资源定义（CRDs）：
...
现在您可以部署其余的 {admc-short-name} 堆栈。官方的 `kubewarden-defaults` helm 图表创建一个名为 `default` 的 PolicyServer。您希望此 PolicyServer 实例启用跟踪。
