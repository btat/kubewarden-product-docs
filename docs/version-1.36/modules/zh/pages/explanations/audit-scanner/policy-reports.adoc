...
:doc-type: [“解释”]
:keywords: ["{admc-name}", “kubewarden”, “kubernetes”, “审计扫描器”, "策略报告"]
:sidebar_label: 策略报告
:current-version: {page-origin-branch}

使用 {admc-name} 审计扫描器时，您使用 https://openreports.io/[OpenReports] 自定义资源存储策略扫描结果。

[CAUTION]
====

在 {admc-name} 1.33 中，审计扫描器功能默认将报告保存到来自 https://openreports.io[openreports.io] 的 OpenReports CRD。
...
操作员可以使用`kubectl`查询报告。他们还可以使用由link:https://kyverno.github.io/policy-reporter[policy-reporter]开源项目提供的可选用户界面来监控并实现 OpenReports CRD 的可观测性。
...
{admc-short-name} 预装了一个过滤器的 Policy Reporter UI，仅显示 {admc-short-name} 策略，并提供展开信息：
