...
En tant qu'opérateur de cluster Kubernetes, vous souhaiterez effectuer des tests pour les stratégies {admc-name} que vous souhaitez utiliser.
...
{admc-short-name} dispose d'un utilitaire, https://github.com/kubewarden/kwctl[`kwctl`], qui permet de tester les stratégies en dehors de Kubernetes.
...
La stratégie {admc-short-name} peut charger depuis le :
