...
= Distribution d'une stratégie de Gatekeeper avec {admc-name}
...
Les stratégies doivent être annotées pour être poussées, et finalement exécutées par le {admc-name} `policy-server` dans un cluster Kubernetes.
