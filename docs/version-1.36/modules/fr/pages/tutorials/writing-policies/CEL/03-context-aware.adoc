...
:description: Configurer des stratégies contextuelles pour garantir des règles d'Ingress uniques entre les hôtes, évitant ainsi les conflits avec les bibliothèques d'extensions CEL de {admc-name}.
...
La {admc-name} de `cel-policy` prend en charge la fonctionnalité xref:reference/spec/05-context-aware-policies.adoc[conscience du contexte]. La stratégie a la capacité de lire les informations du cluster et de prendre des décisions basées sur d'autres ressources existantes en plus de la ressource qui a déclenché l'évaluation de la stratégie via la demande d'admission.

Pour y parvenir, nous pouvons utiliser les https://github.com/kubewarden/cel-policy?tab=readme-ov-file#host-capabilities[bibliothèques d'extensions CEL de {admc-short-name} pour les capacités des hôtes] incluses dans la stratégie.
...
Maintenant, nous devons écrire le code CEL qui récupérera les Ingress existants dans le cluster. Pour cela, nous utilisons la https://github.com/kubewarden/cel-policy?tab=readme-ov-file#host-capabilities[{admc-short-name}bibliothèque d'extensions CEL].
Particulièrement, les capacités d'hôte `kw.k8s`, qui nous permettent d'interroger le cluster pour les GroupVersionKinds. Vous pouvez voir la documentation disponible pour les fonctions CEL https://pkg.go.dev/github.com/kubewarden/cel-policy/internal/cel/library[ici].
