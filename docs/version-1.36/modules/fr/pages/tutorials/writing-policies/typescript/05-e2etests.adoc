...
:keywords: "{admc-name}", kubewarden, kubernetes, tests de stratégies, typescript, tests e2e
:doc-type: didacticiel
:doc-topic: {admc-name}, kubewarden, écriture-des-stratégies, typescript, tests-de-bout-en-bout
...
* `kwctl` >= v1.30 : Outil CLI fourni par {admc-name} pour exécuter ses stratégies en dehors de Kubernetes, entre autres actions. C'est couvert dans xref:/tutorials/testing-policies/index.adoc[la section des tests de stratégies] de la documentation.
...
Le cas de test de bout en bout utilise un fichier JSON contenant un AdmissionReview Kubernetes. Ces objets JSON AdmissionReviews sont ceux qui sont envoyés contre l'API Kubernetes afin qu'ils soient xref:/reference/spec/03-validating-policies.adoc#_an_example[validés].
