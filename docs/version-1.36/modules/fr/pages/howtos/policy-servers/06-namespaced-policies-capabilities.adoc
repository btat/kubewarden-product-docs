...
:keywords: ["{admc-name}", kubernetes, policyserver, admissionpolicy, admissionpolicygroup, host-capabilities, security, namespaced-policies]
:doc-persona: [kubewarden-operator, kubewarden-integrator]
:doc-type: [howto]
:doc-topic: [operator-manual, policy-servers, host-capabilities, namespaced-policies, security]

{admc-name} upholds the following security promise:
...
[WARNING]
====
The per-PolicyServer restriction on host capabilities is part of {admc-name}
`v1.35.0` and higher.
...
xref:/reference/spec/host-capabilities/07-host-capability-call-reference.adoc[capabilities call reference]
documentation page.
...
When upgrading from a {admc-name} version that predates this feature
(`<= v1.34`):
