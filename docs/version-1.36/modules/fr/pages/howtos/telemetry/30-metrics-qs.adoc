...
:description: Configurez le reporting des métriques sur le serveur de stratégie en utilisant Prometheus et {admc-name} activé via des charts Helm pour la collecte de données.
...
Vous pouvez maintenant installer {admc-short-name}, de la manière recommandée avec des charts Helm.
...
Tout d'abord, vous devez ajouter le dépôt Helm qui contient {admc-short-name} :
...
Ensuite, vous installez les définitions de ressources personnalisées (CRDs) définies par {admc-short-name} :
...
Vous pouvez maintenant déployer le reste de la pile {admc-short-name}. Le chart Helm officiel crée un serveur de stratégie nommé `default`.

Vous devez configurer le chart Helm afin d'avoir les métriques activées dans {admc-short-name}. Le fichier `kubewarden-values.yaml` doit avoir le contenu suivant :
...
=== Utiliser le tableau de bord Grafana {admc-short-name}

{admc-short-name} dispose d'un tableau de bord Grafana avec des métriques de base donnant un aperçu de la façon dont {admc-short-name} se comporte dans le cluster. Ce tableau de bord est disponible dans les versions GitHub du dépôt du serveur de stratégie {admc-short-name} sous la forme d'un https://github.com/kubewarden/policy-server/releases/latest/download/kubewarden-dashboard.json[fichier JSON] ou sur le https://grafana.com/grafana/dashboards/15314[site Web de Grafana].
...
Le tableau de bord Grafana a des panneaux montrant l'état de toutes les stratégies gérées par {admc-short-name}. Il a également des panneaux spécifiques aux stratégies.
