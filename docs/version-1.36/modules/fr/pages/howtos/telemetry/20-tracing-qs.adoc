...
Le traçage vous permet de collecter des détails précis sur les évaluations de stratégies. Cela peut être un outil utile pour déboguer des problèmes dans votre {admc-name} déploiement et stratégies.
...
Vous pouvez maintenant procéder au déploiement de {admc-short-name} de la manière habituelle.
...
Comme première étape, vous ajoutez le dépôt Helm qui contient {admc-short-name} :
...
Vous pouvez maintenant déployer le reste de la pile {admc-short-name}. Le chart Helm officiel `kubewarden-defaults` crée un PolicyServer nommé `default`. Vous souhaitez que cette instance de PolicyServer ait le traçage activé.
