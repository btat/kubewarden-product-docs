...
= Installation de kwctl ({admc-name} CLI)
:page-languages: [en, de, es, fr, ja, pt, zh]
:revdate: 2026-05-14
:page-revdate: {revdate}
:title: Installation de kwctl
:description: Installez kwctl (le {admc-name} CLI) en suivant ses instructions d'installation pour divers systèmes d'exploitation, puis activez la complétion de shell.
:keywords: ["{admc-name}", kubewarden, kubernetes, installer kwctl, installer, kwctl]
:doc-persona: kubewarden-all
:doc-type: howto
:doc-topic: install-kwctl

`kwctl` est l'outil d'interface de ligne de commande (CLI) pour {admc-name}. Voici les instructions d'installation pour certains systèmes d'exploitation.
...
. Téléchargez `kwctl` :
    .. Ouvrez votre navigateur et allez sur la page des versions https://github.com/kubewarden/kwctl/releases/latest[{admc-short-name}].
