...
:description: Configurez les politiques {admc-name} dans vos clusters Kubernetes pour obtenir un contrôle granulaire sur la configuration des politiques, remplaçant PodSecurityPolicy (PSP).
...
:doc-type: ["howto"]
:keywords: ["{admc-name}", "kubewarden", "kubernetes", "appvia", "psp", "PodSecurityPolicy"]
...
https://kubernetes.io/docs/concepts/security/pod-security-policy/[PodSecurityPolicy] (PSP) est supprimé. Vous pouvez maintenant utiliser {admc-name} pour le contrôle d'admission sur vos clusters Kubernetes.

{admc-short-name} a des politiques séparées pour atteindre le même objectif qu'une configuration monolithique de PSP. Chaque définition de politique {admc-short-name} fonctionne comme une section de configuration différente dans la spécification d'un PSP. La correspondance des champs de configuration de PSP avec leurs politiques {admc-short-name} respectives se trouve dans le <<_mapping_policies_to_psp_fields,tableau de correspondance>> ci-dessous.

Avec {admc-short-name}, les opérateurs ont un contrôle granulaire de la configuration des politiques dans leurs clusters.

Avec une instance de {admc-short-name}, vous pouvez déployer des politiques pour remplacer l'objet `PodSecurityPolicy`. Nous considérons ces règles dans cet exemple :
...
== {admc-short-name} remplacements pour le PSP

Nous allons maintenant créer des politiques {admc-short-name} pour atteindre le même objectif.
Vous appliquez chaque règle avec une politique {admc-short-name} distincte.
...
[#_mapping_policies_to_psp_fields]
== Mapping des politiques {admc-short-name} aux champs PSP

Ce tableau associe les champs de configuration PSP aux politiques {admc-short-name} correspondantes.

|===
| Champ PSP | Politique équivalente {admc-short-name}
...
L'équipe {admc-short-name} a développé un script pour la migration PSP. Il utilise l'outil de migration de https://github.com/appvia/psp-migration[AppVia]. L'outil AppVia lit une configuration PSP au format YAML. Il génère ensuite les politiques correspondantes. Il le fait pour {admc-short-name} et d'autres moteurs de politique.

[CAUTION]
====

L'outil de migration AppVia est hors du contrôle des mainteneurs de Kubewarden.
Cela signifie qu'il est possible qu'il génère des politiques {admc-short-name} obsolètes. À utiliser avec prudence. Nous avons besoin d'une pull request pour AppVia, sur laquelle le travail est en cours.
Contactez-nous pour plus d'informations si nécessaire.
====


Le script est disponible dans le dépôt {admc-short-name} https://github.com/kubewarden/utils/blob/main/scripts/psp-to-kubewarden[utils]. Il télécharge l'outil de migration AppVia dans le répertoire de travail à utiliser. Il traite les PSP définis dans le contexte par défaut `kubectl`. Ensuite, il imprime les définitions des politiques Kubewarden sur la sortie standard. Les utilisateurs peuvent rediriger le contenu vers un fichier ou vers `kubectl` directement.
...
{admc-short-name}Les politiques peuvent être appliquées directement à un cluster en utilisant la commande suivante :
