...
Par défaut, le contrôleur d'admission ne permet pas aux stratégies d'évaluer les ressources dans son propre espace de noms. Cela empêche les erreurs de configuration susceptibles de compromettre {admc-short-name}l'installation.
...
Pour ce faire, mettez à niveau votre installation de {admc-short-name} et définissez la valeur `alwaysAcceptAdmissionReviewsOnDeploymentsNamespace` du chart Helm `kubewarden-controller` sur `false` :
...
Une fois cela fait, les stratégies peuvent être configurées pour évaluer les examens d'admission de {admc-short-name}l'espace de noms.
