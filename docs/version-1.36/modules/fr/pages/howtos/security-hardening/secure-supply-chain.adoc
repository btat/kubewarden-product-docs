...
:description: Configurez {admc-name} pour valider l'intégrité des signatures des artefacts et des stratégies d'admission.
...
:doc-type: ["howto"]
:doctype: book
:keywords: ["{admc-name}", "kubewarden", "kubernetes", "chaîne d'approvisionnement sécurisée", "infrastructure"]
...
{admc-short-name} utilise https://github.com/sigstore/cosign[`cosign`] avec l'infrastructure https://github.com/SigStore/fulcio[fulcio] et https://github.com/sigstore/rekor[rekor] offerte par le projet Sigstore.

Les opérateurs de cluster peuvent configurer {admc-name} pour n'exécuter que des stratégies signées par des entités de confiance. Les développeurs de stratégies peuvent signer leurs stratégies et les publier dans un registre.
...
Cette commande signe la stratégie en créant un nouvel objet de signature. L'objet de signature est ensuite téléchargé dans le registre, avec la stratégie. Maintenant, la stratégie est prête à être utilisée dans une installation {admc-short-name} utilisant la vérification de signature.
...
Les développeurs de stratégies peuvent utiliser les modèles de stratégies {admc-short-name}. Ils ont des actions GitHub pour construire, tester, signer et publier des stratégies.
...
Vous pouvez configurer {admc-short-name} avec un `ConfigMap` pour n'exécuter que des stratégies de confiance. La structure `ConfigMap` est décrite dans xref:reference/verification-config.adoc#_signature_configuration_reference[Référence de configuration de signature]. Elle est utilisée pour vérifier une stratégie en utilisant `kwctl`. Le `ConfigMap` doit définir les configurations autorisées sous le champ `verification-config`.

Par exemple, vous souhaitez exécuter des stratégies signées par l'organisation GitHub {admc-short-name}. Alors un exemple de `ConfigMap` pour ce scénario serait :
...
Avec {admc-short-name}, vous pouvez vérifier que la signature de la stratégie est faite par des utilisateurs de confiance *et* qu'ils ont des annotations spécifiques.
