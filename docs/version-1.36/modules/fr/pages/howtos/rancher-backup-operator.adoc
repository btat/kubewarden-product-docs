...
:keywords: "{admc-name}", kubernetes, kubewarden, Rancher Backup Operator, sauvegarde, restauration
...
Depuis la version `v9.0.0`, `rancher-backup` prend en charge {admc-name}. Cela inclut :

- L'espace de noms par défaut de Rancher `cattle-kubewarden-system` (ou `cattle-kubewarden-*`), et l'espace de noms par défaut {admc-short-name} `kubewarden`.
- Les ressources nécessaires à {admc-short-name} sont installées via les charts Helm.
- Les CRD {admc-short-name}, qui sont réconciliés après la restauration par le contrôleur {admc-short-name}.
...
Il faut indiquer à Rancher Backup Operator de sauvegarder également les CRs définis par l'utilisateur {admc-short-name} en définissant la valeur `optionalResources.kubewarden.enabled` sur `true`.
...
Utilisez le `rancher-resource-set-full` pour sauvegarder les Secrets {admc-short-name}.
