...
= Qu'est-ce que {admc-name} ?
:page-languages: [en, de, es, fr, ja, pt, zh]
:revdate: 2026-05-04
:page-revdate: {revdate}
:description: Découvrez le rôle de {admc-name} dans la gestion des stratégies Kubernetes, qui inclut l'application de la conformité et l'optimisation des ressources.
...
{admc-name} est un moteur de stratégie Kubernetes.
Il vise à être le moteur de stratégie universel pour Kubernetes.

{intro-paragraph}

== Comment {admc-name} aide-t-il ?

{admc-short-name} offre de la flexibilité pour l'admission et l'application des stratégies dans un environnement Kubernetes.
...
- Le scanner d'audit {admc-short-name} vérifie activement et continuellement l'application des stratégies dans le temps.
...
- {admc-name} fournit une approche complète pour la gestion des stratégies d'admission.
- L'adhésion à la CNCF et une communauté et un écosystème Open Source en croissance aident {admc-short-name} avec la transparence, la collaboration et l'amélioration.
...
== Nouveau sur {admc-short-name} ?

Si vous êtes nouveau dans le projet {admc-short-name}, commencez par le xref:quick-start.adoc[guide de démarrage rapide] et la page xref:explanations/architecture.adoc[architecture].
...
Vous pouvez servir {admc-short-name} stratégies en utilisant un serveur web standard ou, mieux encore, vous pouvez les publier dans un registre conforme à OCI en tant que https://github.com/opencontainers/artifacts[artéfacts OCI].
...
{admc-short-name} est un projet link:https://contribute.cncf.io/maintainers/community/vendor-neutrality/[neutre vis-à-vis des fournisseurs] tel que défini par le CNCF.
