...
La fonctionnalité Kubewarden audit scanner est disponible à partir de la version {admc-name} 1.7.0.

====


Le composant `audit-scanner` vérifie constamment les ressources dans le cluster. Il signale celles qui ne respectent pas les stratégies {admc-short-name} déployées dans le cluster.
...
Pour expliquer l'utilisation du Kubewarden audit scanner dans {admc-short-name}, considérons le scénario suivant.
...
Quelques jours plus tard, Alice, l'administratrice Kubernetes, applique une stratégie {admc-short-name} qui interdit la création de conteneurs privilégiés. Le Pod déployé par Bob continue de fonctionner dans le cluster car il existe déjà.
...
Vous pouvez activer le Kubewarden audit scanner à partir de la version {admc-short-name} 1.7.0.
...
De plus, les stratégies dans {admc-short-name} prennent désormais en charge deux annotations optionnelles :
...
Le Kubewarden audit scanner se lie à un `ClusterRole` qui accorde un accès en lecture aux types de ressources {admc-short-name} et un accès en lecture-écriture aux CRDs `PolicyReport`.  Ces autorisations permettent au Kubewarden audit scanner de récupérer des ressources pour effectuer des évaluations d'audit et créer des rapports de stratégie basés sur les résultats de l'évaluation.
