...
:description: Distribuez les stratégies de manière sécurisée via un registre conforme à l'OCI en utilisant des outils comme kwctl, et assurez la sécurité du cluster grâce à {admc-name} la gestion des stratégies.
...
:keywords: ["{admc-name}", "oci", "kubewarden", "policy", "wasm", "webassembly"]
:current-version: {page-origin-branch}

Les stratégies {admc-name} sont des binaires WebAssembly (Wasm) évalués par le serveur de stratégie {admc-short-name}.

Le {admc-short-name} serveur de stratégie charge les stratégies à partir de ces sources :
...
{admc-short-name} recommande de distribuer les stratégies {admc-short-name} via un registre conforme à l'OCI. Les registres de conteneurs sont une exigence obligatoire pour tout cluster Kubernetes. Avoir un seul endroit pour stocker et sécuriser tous les artefacts requis par un cluster est bénéfique.
...
Le registre conforme à l'OCI *doit prendre en charge les artefacts* pour pouvoir y pousser avec succès une {admc-short-name} stratégie.

Vous pouvez utiliser https://github.com/kubewarden/kwctl[`kwctl`] CLI pour pousser une {admc-short-name} stratégie vers un registre conforme à l'OCI.

== Annotation de la stratégie

Vous annotez également une stratégie avec `kwctl`. L'annotation d'une stratégie {admc-short-name} se fait en ajoutant des sections personnalisées Wasm au binaire de la stratégie. Cela signifie que la stratégie emballe ses propres métadonnées.

La commande `kwctl annotate` nécessite deux entrées :

* la stratégie {admc-short-name} à annoter, un fichier local dans le système de fichiers.
...
Vous pouvez faire référence à une stratégie depuis le {admc-short-name} serveur de stratégie ou `kwctl` en tant que `registry://<oci-registry>/kubewarden-policies/palindromify-policy:v0.0.1`.
