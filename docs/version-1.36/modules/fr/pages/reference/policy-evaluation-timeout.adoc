...
Cette fonctionnalité est disponible à partir de {admc-name} v1.5.0 pour le délai par serveur de stratégies et à partir de v1.29.0 pour le délai par stratégie.
...
À partir de {admc-short-name} v1.29.0, chaque {admc-short-name} stratégie peut définir sa propre valeur de délai via son attribut `spec.timeoutEvalSeconds`. Cela ne doit pas être confondu avec `spec.timeoutSeconds`, utilisé pour le délai des webhooks (voir la section <<_comparison_with_kubernetes_dynamic_admission_controller_timeout,ci-dessous>>).
...
{admc-short-name} est une implémentation de https://en.wikipedia.org/wiki/Webhook[webhook] du https://kubernetes.io/docs/reference/access-authn-authz/extensible-admission-controllers/[Contrôleur d'Admission Dynamique Kubernetes].

En interne, le serveur API Kubernetes effectue une requête HTTP vers le serveur de stratégies de {admc-short-name} décrivant un événement sur le point de se produire. Après la requête HTTP, le Serveur API Kubernetes attend une réponse. Cependant, le serveur API Kubernetes n'attend pas indéfiniment. Après un certain temps, il considère que la requête a expiré.
...
Chaque stratégie {admc-short-name} peut définir sa propre valeur de délai d'attente via l'attribut `timeoutSeconds` des ressources personnalisées `ClusterAdmissionPolicy` et `AdmissionPolicy`.
...
Vous pouvez maintenant examiner les scénarios suivants pour mieux comprendre les différences entre le délai d'expiration du Webhook de Kubernetes et le délai d'évaluation de la stratégie de {admc-short-name}.


=== {admc-short-name} Le délai d'évaluation de la stratégie est désactivé.
...
=== {admc-short-name} Le délai d'évaluation de la stratégie est activé.
...
Après deux secondes, la fonction de délai d'évaluation de la stratégie de {admc-short-name} interrompt l'évaluation de la stratégie et produit une réponse de rejet. La réponse contient un message expliquant que le rejet s'est produit parce que l'évaluation de la stratégie n'a pas été complétée à temps.

[NOTE]
====

Définir le délai d'évaluation de la stratégie de {admc-short-name} à une valeur aussi élevée que le délai d'expiration du webhook de Kubernetes n'est pas un bon choix.
