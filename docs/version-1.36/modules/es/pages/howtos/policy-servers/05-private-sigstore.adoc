...
:description: Configurando PolicyServers para utilizar una instancia privada o autohospedada de Sigstore para la verificación de firmas de políticas en {admc-name}.
...
Asegúrate de descargar de forma segura los certificados y claves de tu instancia de Sigstore. De lo contrario, podrías acabar configurando {admc-name} con datos comprometidos.
...
[NOTE]
====
La clave del ConfigMap **debe** ser `verification-config` (no el nombre del archivo). {admc-short-name} busca esta clave exacta.
====
