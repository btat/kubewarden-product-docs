...
:description: Configura la generación de métricas en el Servidor de Políticas utilizando Prometheus y {admc-name} habilitado a través de gráficos de Helm para la recolección de datos.
...
Ahora puedes instalar {admc-short-name}, de la manera recomendada con gráficos de Helm.
...
Primero, deberías añadir el repositorio de Helm que contiene {admc-short-name}:
...
Luego, instalas las Definiciones de Recursos Personalizados (CRDs) definidas por {admc-short-name}:
...
Ahora puedes desplegar el resto de la pila de {admc-short-name}. El chart oficial de Helm crea un PolicyServer llamado `default`.

Deberías configurar el chart de Helm para que tengas métricas habilitadas en {admc-short-name}. El archivo `kubewarden-values.yaml` debería tener el siguiente contenido:
...
=== Usando el {admc-short-name}panel de Grafana

{admc-short-name} tiene un panel de Grafana con métricas básicas que ofrecen una visión general de cómo se comporta {admc-short-name} en el clúster. Este panel de control está disponible en las versiones de GitHub del repositorio de {admc-short-name} policy-server como un https://github.com/kubewarden/policy-server/releases/latest/download/kubewarden-dashboard.json[archivo JSON] o en el https://grafana.com/grafana/dashboards/15314[sitio web de Grafana].
...
El panel de Grafana tiene paneles que muestran el estado de todas las directivas gestionadas por {admc-short-name}. También tiene paneles específicos de directivas.
