...
El trazado te permite recopilar detalles precisos sobre las evaluaciones de políticas. Puede ser una herramienta útil para depurar problemas en tu ampliación de {admc-name}
y políticas.
...
Ahora puedes proceder a la ampliación de {admc-short-name} de la manera habitual.
...
Como primer paso, añades el repositorio de Helm que contiene {admc-short-name}:
...
Luego instalas las definiciones de recursos personalizadas (CRDs) definidas por {admc-short-name}:
...
Ahora puedes desplegar el resto de la pila de {admc-short-name}. El chart de Helm oficial de `kubewarden-defaults` crea un PolicyServer llamado `default`. Quieres que esta instancia de PolicyServer tenga el trazado habilitado.
