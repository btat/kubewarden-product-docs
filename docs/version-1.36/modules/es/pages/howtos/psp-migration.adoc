...
:description: Configura {admc-name} directivas en tus clústeres de Kubernetes para lograr un control granular sobre la configuración de directivas, reemplazando PodSecurityPolicy (PSP).
...
https://kubernetes.io/docs/concepts/security/pod-security-policy/[PodSecurityPolicy] (PSP) es eliminado. Ahora puedes usar {admc-name} para el control de admisión en tus clústeres de Kubernetes.

{admc-short-name} tiene políticas separadas para lograr el mismo objetivo que una configuración monolítica de PSP. Cada definición de directiva {admc-short-name} funciona como una sección de configuración diferente en la especificación de un PSP. El mapeo de los campos de configuración de PSP a sus respectivas directivas {admc-short-name} está en la <<_mapping_policies_to_psp_fields,tabla de mapeo>> a continuación.

Con {admc-short-name}, los operadores tienen control granular de la configuración de directivas en sus clústeres.

Con una instancia de {admc-short-name}, puedes desplegar directivas para reemplazar el objeto `PodSecurityPolicy`. Consideramos estas reglas en este ejemplo:
...
== {admc-short-name} reemplazos para PSP

Ahora crearemos {admc-short-name} directivas para lograr el mismo objetivo.
Se aplica cada regla con una directiva {admc-short-name} separada.
...
== Asignando políticas de {admc-short-name} a campos de PSP

Esta tabla asigna los campos de configuración de PSP a las políticas correspondientes de {admc-short-name}.

|===
| Campo de PSP | Política equivalente a {admc-short-name}
...
El equipo {admc-short-name} ha desarrollado un script para la migración de PSP. Utiliza la herramienta de migración de https://github.com/appvia/psp-migration[AppVia]. Luego genera las directivas correspondientes. Luego genera las políticas correspondientes. Lo hace para {admc-short-name} y otros motores de políticas.

[CAUTION]
====

La herramienta de migración AppVia está fuera del control de los mantenedores de Kubewarden.
Esto significa que es posible que genere políticas {admc-short-name} desactualizadas. Utilizar con precaución. Necesitamos una solicitud de extracción para AppVia, para la cual se está trabajando.
Contáctanos para más información si lo necesitas.
====


El script está disponible en el {admc-short-name}
...
{admc-short-name} las directivas se pueden aplicar directamente a un clúster utilizando el siguiente comando:
