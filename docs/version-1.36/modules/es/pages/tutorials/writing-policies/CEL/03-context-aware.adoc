...
:description: Configura políticas conscientes del contexto para asegurar reglas de Ingress únicas entre hosts, evitando conflictos con las bibliotecas de extensiones CEL de {admc-name}.
...
La {admc-name} de `cel-policy` soporta la característica de xref:reference/spec/05-context-aware-policies.adoc[conciencia del contexto]. La política tiene la capacidad de leer información del clúster y tomar decisiones basadas en otros recursos existentes además del recurso que activó la evaluación de la política a través de la solicitud de admisión.

Para lograr esto, podemos usar las https://github.com/kubewarden/cel-policy?tab=readme-ov-file#host-capabilities[bibliotecas de extensiones CEL de {admc-short-name} para capacidades de host] incluidas en la política.
...
Ahora, necesitamos escribir el código CEL que obtendrá los Ingresses existentes en el clúster. Para eso, usamos la https://github.com/kubewarden/cel-policy?tab=readme-ov-file#host-capabilities[{admc-short-name} biblioteca de extensiones CEL].
Particularmente, las `kw.k8s` capacidades de host, que nos permiten consultar el clúster por GroupVersionKinds. Puedes ver la documentación disponible para las funciones CEL https://pkg.go.dev/github.com/kubewarden/cel-policy/internal/cel/library[aquí].
