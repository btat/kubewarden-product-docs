...
:keywords: "{admc-name}", kubewarden, kubernetes, pruebas de políticas, typescript, pruebas e2e
:doc-type: tutorial
:doc-topic: {admc-name}, kubewarden, escritura-de-políticas, typescript, pruebas-de-extremo-a-extremo
...
* `kwctl` >= v1.30: Herramienta CLI proporcionada por {admc-name} para ejecutar su directiva fuera de Kubernetes, entre otras acciones. Está cubierto en xref:/tutorials/testing-policies/index.adoc[la sección de pruebas de directivas] de la documentación.
...
El caso de prueba de extremo a extremo utiliza un archivo JSON que contiene un Kubernetes AdmissionReview. Estos objetos JSON de AdmissionReviews son los que se envían contra la API de Kubernetes para que sean xref:/reference/spec/03-validating-policies.adoc#_an_example[validados].
...
Las pruebas de extremo a extremo proporcionan una cobertura completa del comportamiento de la directiva al probar contra el binario de WebAssembly real. Esto asegura que la directiva funcione correctamente cuando se despliega en {admc-short-name}, no solo en el entorno de desarrollo de TypeScript.

[TIP]
====
Para ejemplos más completos de pruebas de extremo a extremo, puedes explorar el https://github.com/kubewarden/policy-sdk-js/blob/main/demo_policy/e2e.bats[código fuente de policy-sdk-js], que incluye extensas pruebas e2e que demuestran cada una de las capacidades de host de {admc-short-name}. Estas pruebas pueden servir de inspiración para probar características de directiva más avanzadas, como redes, operaciones criptográficas e interacciones con el registro OCI.
====