...
La función Audit Scanner está disponible a partir de la versión {admc-name} 1.7.0

====


El componente `audit-scanner` comprueba constantemente los recursos en el clúster. Marca aquellos que no cumplen con las políticas {admc-short-name} desplegadas en el clúster.
...
Para explicar el uso del Audit Scanner en {admc-short-name}, considera el siguiente escenario.
...
Unos días después, Alice, la administradora de Kubernetes, aplica una política {admc-short-name} que prohíbe la creación de contenedores privilegiados. El Pod desplegado por Bob sigue ejecutándose en el clúster ya que ya existe.
...
Puedes habilitar el Audit Scanner a partir de la versión {admc-short-name} 1.7.0.
...
Además, las políticas en {admc-short-name} ahora admiten dos anotaciones opcionales:
