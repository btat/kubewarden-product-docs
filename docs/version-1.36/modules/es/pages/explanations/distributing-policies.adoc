...
:description: Distribuye directivas de forma segura a través de un registro compatible con OCI utilizando herramientas como kwctl, y asegura la seguridad del clúster mediante {admc-name} la gestión de directivas.
...
Las directivas {admc-name} son binarios de WebAssembly (Wasm) evaluados por el servidor de directivas {admc-short-name}.

El servidor de directivas {admc-short-name} carga directivas de estas fuentes:
...
{admc-short-name} recomienda distribuir directivas {admc-short-name} a través de un registro compatible con OCI. Los registros de contenedores son un requisito obligatorio para cualquier clúster de Kubernetes. Tener un único lugar para almacenar y asegurar todos los artefactos requeridos por un clúster es beneficioso.
...
El registro compatible con OCI *debe soportar artefactos* para poder enviar con éxito una {admc-short-name} directiva a él.

Puedes usar la CLI https://github.com/kubewarden/kwctl[`kwctl`] para enviar una {admc-short-name} directiva a un registro compatible con OCI.

== Anotando la directiva

También anotas una directiva con `kwctl`. Anotar una directiva {admc-short-name} se realiza añadiendo secciones personalizadas de Wasm al binario de la directiva. Esto significa que la directiva empaqueta su propia metadata.

El comando `kwctl annotate` requiere dos entradas:

* la directiva {admc-short-name} a anotar, un archivo local en el sistema de archivos.
...
Puedes hacer referencia a una directiva desde el Servidor de Directivas {admc-short-name} o `kwctl` como
