...
:description: Aprende sobre el papel de {admc-name} en la gestión de políticas de Kubernetes, que incluye garantizar el cumplimiento y optimizar recursos.
...
{admc-name} es un motor de políticas de Kubernetes.
Aspira a ser el motor de políticas universal para Kubernetes.

{intro-paragraph}

== ¿Cómo ayuda {admc-name}?

{admc-short-name} ofrece flexibilidad para la admisión y aplicación de políticas en un entorno de Kubernetes.
...
- El escáner de auditoría de {admc-short-name} verifica de forma activa y continua la aplicación de políticas a lo largo del tiempo.
...
- {admc-name} proporciona un enfoque integral para la gestión de políticas de admisión.
- La membresía en CNCF y una creciente comunidad y ecosistema de código abierto ayudan a {admc-short-name} con transparencia, colaboración y mejora.
...
== ¿Nuevo en {admc-short-name}?

Si eres nuevo en el proyecto {admc-short-name}, comienza con la xref:quick-start.adoc[guía de inicio rápido] y la página de xref:explanations/architecture.adoc[arquitectura].
...
Puedes servir {admc-short-name} políticas utilizando un servidor web estándar o, mejor aún, puedes publicarlas en un registro compatible con OCI como https://github.com/opencontainers/artifacts[artefactos OCI].
...
{admc-short-name} es un proyecto https://contribute.cncf.io/maintainers/community/vendor-neutrality/[neutral al proveedor] según lo definido por la CNCF.
