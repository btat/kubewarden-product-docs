...
:description: ポリシー署名検証のために、{admc-name}でプライベートまたは自己ホスト型のSigstoreインスタンスを使用するようにPolicyServersを設定します。
...
* ローカルにインストールされたlink:https://docs.sigstore.dev/cosign/system_config/installation/[`cosign`] CLI。
* ローカルにインストールされたlink:https://jqlang.org/[`jq`]
...
Sigstoreインスタンスから証明書とキーを安全にダウンロードしてください。そうしないと、{admc-name}が改ざんされたデータで構成される可能性があります。
...
両方のコマンドのすべての利用可能なフラグとJSONフィールド定義の完全な説明については、link:https://docs.sigstore.dev/cosign/system_config/installation/[cosign CLIリファレンス]およびlink:https://github.com/sigstore/protobuf-specs[Sigstoreクライアント信頼設定仕様]を参照してください。
...
[NOTE]
====
ConfigMap キー**は**`verification-config`でなければなりません（ファイル名ではありません）。{admc-short-name}はこの正確なキーを探します。
====
