...
:doc-type: ["howto"]
:keywords: ["{admc-name}", "kubewarden", "kubernetes", "policyservers", "private registries"]
...
[#_creating_the_secret]
== シークレットの作成

PolicyServersは通常のlink:https://kubernetes.io/docs/concepts/configuration/secret/#docker-config-secrets[Docker設定シークレット]をサポートしており、タイプ`kubernetes.io/dockercfg`またはタイプ`kubernetes.io/dockerconfigjson`のいずれかです。これらのシークレットは`kubectl create secret docker-registry`を使用して作成できます。
...
Dockerシークレットの作成方法についての詳細は、link:https://kubernetes.io/docs/concepts/configuration/secret/#docker-config-secrets[Kubernetesドキュメント]を参照してください。
