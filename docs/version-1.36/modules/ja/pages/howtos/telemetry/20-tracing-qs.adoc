...
トレーシングを使用すると、ポリシー評価に関する詳細な情報を収集できます。これは、{admc-name}のデプロイメントおよびポリシーの問題をデバッグするための便利なツールとなる可能性があります。
...
執筆時点（2022-06-21）では、特定のバージョンのJaegerのみがCert Managerと互換性があります。link:https://github.com/jaegertracing/helm-charts/blob/main/charts/jaeger-operator/COMPATIBILITY.md[互換性チャートを参照してください]。
...
これは*本番環境のデプロイメントには適していません*。Jaegerのlink:https://www.jaegertracing.io/docs/latest/kubernetes/[公式ドキュメント]を参照してください。
...
これで、通常の方法で{admc-short-name}のデプロイメントを進めることができます。
...
最初のステップとして、{admc-short-name}を含むHelmリポジトリを追加します：
...
次に、{admc-short-name}によって定義されたカスタムリソース定義をインストールします：
...
これで、{admc-short-name}スタックの残りをデプロイすることができます。公式の`kubewarden-defaults` Helmチャートは、`default`という名前のPolicyServerを作成します。このPolicyServerインスタンスにはトレーシングを有効にしたいです。
...
再度、これは*本番環境でのデプロイには不適切です*。Jaegerのlink:https://www.jaegertracing.io/docs/latest/kubernetes/[公式ドキュメント]を参照してください。
