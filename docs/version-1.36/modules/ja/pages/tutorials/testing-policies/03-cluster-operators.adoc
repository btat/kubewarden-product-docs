...
Kubernetesクラスターオペレーターとして、使用したい{admc-name}ポリシーのテストを行いたいと思うでしょう。
...
{admc-short-name}には、Kubernetesの外部でポリシーのテストを許可するユーティリティ、link:https://github.com/kubewarden/kwctl[`kwctl`]があります。

`kwctl`を使用するには、次の入力で呼び出します：

. 実行するポリシーのWebAssemblyバイナリファイルURI。
{admc-short-name}ポリシーは次の場所から読み込むことができます：
 ** ローカルファイルシステム `file://`
 ** HTTP(S)サーバー `https://`
 ** OCIレジストリ `registry://`。
...
このセクションでは、異なる設定と検証リクエストオブジェクトを使用して、link:https://github.com/kubewarden/psp-apparmor[psp-apparmor]ポリシーをテストする方法について説明します。
