...
. JSONクエリを実行します（link:https://stedolan.github.io/jq/[`jq`]に似たもの）。
...
その後の呼び出しは速く、{admc-name}がWasmtimeのキャッシュ機能を使用するためです。
最初の実行には約20秒かかる場合があり、以降の実行は1〜2秒です。
したがって、{admc-short-name}ポリシーサーバーは起動時間が遅いですが、その後のポリシー評価時間は通常Kubernetesオブジェクトの使用によって影響を受けません。
====
...
. `payload`オブジェクトを作成します。{admc-short-name} SDKのヘルパー関数を使用してこれを行います：`kubewarden_testing.BuildValidationRequest`。
...
ポリシーが{admc-short-name}で評価されるときにはこれが発生せず、そのコンテキストでは`logger`ステートメントがOpenTelemetryイベントを発生させます。
====
