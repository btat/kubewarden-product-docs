...
正規表現オブジェクトを処理するために`regexp`パッケージを使用しており、拒否されたラベルのリストを格納するためにlink:https://github.com/deckarep/golang-set[`mapset.Set`]型を使用しています。
...
{admc-short-name}ポリシーは、ポリシー設定を入力として受け取る2つの異なる関数を公開します：

* `validate`:Kubernetesオブジェクトがポリシーによる検証を必要とする場合は、この関数を使用してください。
設定は、link:https://pkg.go.dev/github.com/kubewarden/policy-sdk-go@v0.2.1/protocol#ValidationRequest[`ValidationRequest`]オブジェクトの一部です。
* `validate_settings`:ポリシーが最初に{admc-short-name}によって読み込まれたときにこの関数を呼び出します。
...
[NOTE]
====
すべての`mapset.Set`オブジェクトは、link:https://pkg.go.dev/github.com/deckarep/golang-set?utm_source=godoc#NewThreadUnsafeSet[スレッド非安全なバリアント]を使用して作成されます。
WebAssemblyコードは単一スレッドで実行されるため、競合の問題はありません。

WebAssembly標準はまだスレッドをカバーしていません。
詳細については、link:https://github.com/WebAssembly/threads[公式提案]を参照してください。
====


== `Settings`の検証を実装する

すべての{admc-short-name}ポリシーは、設定の検証を実装する必要があります。
...
この関数は、link:https://github.com/kubewarden/policy-sdk-go[{admc-short-name}のSDK]によって提供されるヘルパー関数を活用していることがわかります。
