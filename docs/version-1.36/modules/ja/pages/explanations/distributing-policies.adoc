...
:description: kwctlのようなOCI準拠のレジストリを介してポリシーを安全に配布し、{admc-name}ポリシー管理を通じてクラスターのセキュリティを確保します。
...
{admc-name}ポリシーは、{admc-short-name}ポリシーサーバーによって評価されるWebAssembly（Wasm）バイナリです。

{admc-short-name}ポリシーサーバーは、これらのソースからポリシーを読み込みます：
...
{admc-short-name}は、通常のOCI準拠のレジストリを介して{admc-short-name}ポリシーを配布することを推奨します。コンテナレジストリは、すべてのKubernetesクラスターにとって必須の要件です。クラスターに必要なすべてのアーティファクトを保存し、保護するための単一の場所を持つことは有益です。
...
*ターゲットOCI準拠のレジストリは、アーティファクトをサポートする必要があります*。これにより、{admc-short-name}ポリシーを正常にプッシュできます。

https://github.com/kubewarden/kwctl[`kwctl`] CLIを使用して、{admc-short-name}ポリシーをOCI準拠のレジストリにプッシュできます。

== ポリシーの注釈付け

ポリシーには、`kwctl`を使用して注釈を付けることもできます。ポリシーの注釈付けは、{admc-short-name}ポリシーのバイナリにWasmカスタムセクションを追加することで行われます。これは、ポリシーが独自のメタデータをパッケージ化することを意味します。

`kwctl annotate`コマンドは、2つの入力を必要とします：

* {admc-short-name}ポリシーに注釈を付けるための、ファイルシステム内のローカルファイル。
...
{admc-short-name}ポリシーサーバーまたは`kwctl`から`registry://<oci-registry>/kubewarden-policies/palindromify-policy:v0.0.1`としてポリシーを参照できます。
