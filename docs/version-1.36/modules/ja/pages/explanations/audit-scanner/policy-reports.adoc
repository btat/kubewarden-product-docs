...
:keywords: ["{admc-name}", "kubewarden", "kubernetes", "監査スキャナー", "ポリシーレポート"]
:sidebar_label: ポリシーレポート
:current-version: {page-origin-branch}

{admc-name}監査スキャナーを使用する際、link:https://openreports.io/[OpenReports]カスタムリソースを使用してポリシースキャンの結果を保存します。

[CAUTION]
====

{admc-name} 1.33では、監査スキャナー機能がデフォルトでlink:https://openreports.io[openreports.io]のOpenReports CRDにレポートを保存します。
...
オペレーターは`kubectl`を使用してレポートを照会できます。彼らはまた、link:https://kyverno.github.io/policy-reporter[policy-reporter]オープンソースプロジェクトが提供するオプションのUIを使用して、OpenReports CRDのモニタリングと監視を行うことができます。
...
{admc-short-name}は、{admc-short-name}ポリシーのみを表示する事前設定済みのフィルターを備えたポリシーレポータUIを搭載し、拡張情報を提供します。

image::policy-reporter_policyreports.png[ポリシーレポータ ポリシーレポートの例]

ポリシーレポータのその他の機能には、結果を異なるクライアント（Grafana Loki、Elasticsearch、チャットアプリケーションなど）に転送すること、メトリックエンドポイント、その他が含まれます。詳細については、link:https://kyverno.github.io/policy-reporter[ポリシーレポータのコミュニティドキュメント]をご覧ください。
