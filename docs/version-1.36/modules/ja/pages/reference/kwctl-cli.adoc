...
:title: kwctl CLI
:description: \`kwctl` コマンドラインツールを探索して、{admc-name} ポリシーを管理し、ポリシーの読み込みなどのタスクを実行します。
...
ポリシーを管理する{admc-name} ツール

*使用方法:* `kwctl [OPTIONS] <COMMAND>`

==== サブコマンド

* `annotate` — WebAssembly モジュールに {admc-short-name} メタデータを追加します。
* `bench` — {admc-short-name} ポリシーのベンチマークを実行します。
* `completions` — シェルの補完を生成します。
* `digest` — ポリシーの OCI マニフェストからダイジェストを取得します。
* `docs` — kwctl コマンドのマークダウンドキュメントを生成します。
* `info` — システム情報を表示します。
* `inspect` — {admc-short-name} ポリシーを検査します。
* `load` — tar.gz ファイルからポリシーを読み込みます。
* `policies` — ダウンロードしたすべてのポリシーをリストします。
* `pull` — 指定された URI から {admc-short-name} ポリシーを取得します。
* `push` — OCI レジストリに {admc-short-name} ポリシーをプッシュします。
* `rm` — ストアから {admc-short-name} ポリシーを削除します。
* `run` — 指定された URI から {admc-short-name} ポリシーを実行します
* `save` — ポリシーを tar.gz ファイルに保存します
* `scaffold` — Kubernetes リソースまたは設定ファイルを作成します。
* `verify` — Sigstore を使用して指定された URI から {admc-short-name} ポリシーを検証します
...
WebAssembly モジュールに {admc-short-name} メタデータを追加します
...
{admc-short-name}ポリシーのベンチマークを実行します。
...
- ローカル YAML ファイル: 例、`file://home/tux/cluster-admission-policy.yaml`（{admc-short-name} カスタムリソースの宣言を含む、`ClusterAdmissionPolicy`、`AdmissionPolicy` など）

デフォルトの動作:
スキーマが省略された場合、`file://` が仮定され、現在のディレクトリをルートとして扱われます。

{admc-short-name} カスタムリソースに関する注意事項：
...
- `--raw` フラグは使用できません。{admc-short-name} のカスタムリソースは `raw` ポリシーをサポートしていません。
...
* `<URI_OR_SHA_PREFIX_OR_YAML_FILE>` — ポリシーのURI、SHA プレフィックス、または{admc-short-name}ポリシーリソースを含むYAMLファイル。サポートされているスキーム: registry://, https://, file://。スキーマが省略された場合、file:// が仮定され、現在のディレクトリをルートとして扱われます。
...
指定されたURIから{admc-short-name}ポリシーをプルします
...
* `--github-owner <VALUE>` — CDパイプラインで生成された証明書に期待されるGitHubオーナー
* `--github-repo <VALUE>` — CDパイプラインで生成された証明書に期待されるGitHubリポジトリ
* `-o`, `--output-path <PATH>` — 出力ファイル。提供されていない場合は{admc-short-name}ストアにダウンロードされます
* `--sigstore-trust-config <PATH>` — Sigstore protobuf 仕様の ClientTrustConfig メッセージに準拠した JSON 形式のファイル。このファイルは、CA およびアーティファクトの透明性サービスにアクセスするために使用される URI や、信頼の根となる暗号的なルートを含む、Sigstore インスタンス全体の状態を構成します。
* `--sources-path <PATH>` — ソース情報を保持する YAML ファイル (https、レジストリの不正なホスト、カスタム CA...)
* `-a`, `--verification-annotation <KEY=VALUE>` — key=value 形式の注釈。複数回繰り返すことができます。
* `--verification-config-path <PATH>` — 署名、公開鍵...に関する検証設定情報を保持する YAML ファイル。
...
{admc-short-name}ポリシーをOCIレジストリにプッシュします
...
ストアから{admc-short-name}ポリシーを削除します。
...
1つ以上の{admc-short-name}ポリシーをローカルで実行します。
...
* `<URI_OR_SHA_PREFIX_OR_YAML_FILE>` — ポリシーのURI、SHAプレフィックス、または{admc-short-name}ポリシーリソースを含むYAMLファイル。サポートされているスキーム: registry://, https://, file://。スキーマが省略された場合、file://が仮定され、現在のディレクトリがルートになります。
...
* `vap` — Kubernetes `ValidatingAdmissionPolicy`を{admc-short-name} `ClusterAdmissionPolicy`に変換します。
...
* `-o`, `--operation <TYPE>` — {admc-short-name} カスタムリソースタイプ +
  指定可能な値: `CREATE`
...
* `-t`, `--type <VALUE>` — {admc-short-name} カスタムリソースタイプ +
...
Kubernetes `ValidatingAdmissionPolicy`を{admc-short-name} `ClusterAdmissionPolicy`に変換します。
...
指定されたURIからSigstoreを使用して{admc-short-name}ポリシーを検証します。
...
この文書はlink:https://crates.io/crates/clap-markdown[`clap-markdown`]によって自動的に生成されました。
