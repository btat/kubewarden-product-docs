...
* `githubAction`:GitHub Actions内でSigstoreのキーレスワークフローで行われた署名のため。{admc-short-name}は、GitHubのOpenID Connectによって作成されたx509証明書拡張`workflow_repository`に対してこの情報を確認し、`issuer`と`subject`だけでなく、他の情報も確認します。GitHub Actionsを扱う場合は、この`kind`を使用するべきです。
...
{admc-short-name}を使用すると、ポリシー署名が信頼できるユーザーによるものであり、さらに特定の注釈が付与されていることを確認できます。
