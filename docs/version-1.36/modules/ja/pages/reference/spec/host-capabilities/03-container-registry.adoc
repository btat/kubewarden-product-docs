...
OCIアーティファクトは、{admc-name} ポリシー、Helmチャートなどのオブジェクトを保存するために使用されます。

これらは、コンテナレジストリと対話するために {admc-short-name} ホストによって公開される機能です。
...
ダイジェストの計算には、{admc-short-name}ポリシーホストとリモートレジストリ間の一連のネットワークリクエストが関与します。
これらの操作は時間がかかる場合があるため、結果は1分間キャッシュされます。

=== 認証

プライベートレジストリとの対話には、{admc-short-name}ポリシーホストがリモートレジストリに対して認証を行う必要があります。
...
この情報を取得するには、{admc-short-name}ポリシーホストとリモートレジストリ間でネットワークリクエストが必要です。これらの操作は時間がかかる場合があるため、結果は1分間キャッシュされます。

=== 認証

プライベートレジストリとのやり取りには、{admc-short-name}ポリシーホストがリモートレジストリに対して認証を行う必要があります。
...
それはlink:https://github.com/opencontainers/image-spec/blob/main/image-index.md[OCIインデックスイメージ]またはlink:https://github.com/opencontainers/image-spec/blob/main/manifest.md[OCIイメージマニフェスト]である可能性があります。
...
* 出力ペイロード: レジストリから取得した成功した応答の本文。それにはlink:https://github.com/opencontainers/image-spec/blob/main/manifest.md[OCIイメージマニフェスト]、イメージダイジェスト、およびlink:https://github.com/opencontainers/image-spec/blob/main/config.md[OCIイメージ設定]が含まれます。
