...
{admc-name}はlink:https://www.sigstore.dev/[Sigstore]プロジェクトのサポートを実装します。
...
それらはコンテナイメージまたはより一般的なOCIアーティファクト、例えば{admc-short-name}ポリシーのようなものです。
...
したがって、これはWebAssemblyランタイム、すなわち{admc-short-name}の`policy-server`または`kwctl`によって行われます。

{admc-short-name}ポリシー向けの各種言語SDKがこれを処理します。
それらは、コンテナイメージ、{admc-short-name}ポリシー、Helmチャート、その他の種類のOCIアーティファクトの検証のための関数を提供します。
...
{admc-short-name}チームは、link:https://github.com/kubewarden/verify-image-signatures[検証ポリシー]を提供し、すべてのコンテナに対してSigstore署名を強制します。
