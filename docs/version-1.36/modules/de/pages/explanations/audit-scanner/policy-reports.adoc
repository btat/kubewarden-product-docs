...
:doc-type: ["erklärung"]
:keywords: ["{admc-name}", "kubewarden", "kubernetes", "audit scanner", "richtlinienberichte"]
:sidebar_label: Richtlinienberichte
:current-version: {page-origin-branch}

Wenn Sie den {admc-name} Audit Scanner verwenden, speichern Sie die Ergebnisse der Richtlinienprüfungen mithilfe der https://openreports.io/[OpenReports] Benutzerdefinierten Ressourcen.

[CAUTION]
====

Mit {admc-name} 1.33 speichert die Audit Scanner-Funktion die Berichte standardmäßig in OpenReports CRDs von https://openreports.io[openreports.io].
...
{admc-short-name} liefert die Benutzeroberfläche des Policy Reporters mit einem vorkonfigurierten Filter aus, der nur {admc-short-name} Richtlinien anzeigt, zusammen mit erweiterten Informationen:
