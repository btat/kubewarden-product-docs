...
:keywords: ["{admc-name}", "kubewarden", "audit scanner", "kubernetes"]
...
Die Funktion des Kubewarden audit scanner ist ab der Version {admc-name} 1.7.0 verfügbar.

====


Die `audit-scanner`-Komponente überprüft ständig die Ressourcen im Cluster. Sie kennzeichnet die Ressourcen, die nicht den im Cluster bereitgestellten {admc-short-name}-Richtlinien entsprechen.
...
Um die Verwendung des Kubewarden audit scanners in {admc-short-name} zu erklären, betrachten Sie das folgende Szenario.
...
Einige Tage später setzt Alice, die Kubernetes-Administratorin, eine {admc-short-name}-Richtlinie durch, die die Erstellung privilegierter Container verbietet. Der von Bob bereitgestellte Pod läuft weiterhin im Cluster, da er bereits existiert.
...
Sie können den Kubewarden audit scanner ab der Version {admc-short-name} 1.7.0 aktivieren.
...
Außerdem unterstützen Richtlinien in {admc-short-name} jetzt zwei optionale Annotationen:
...
Der Kubewarden audit scanner bindet an ein `ClusterRole`, das Lesezugriff auf {admc-short-name} Ressourcentypen und Lese- und Schreibzugriff auf die `PolicyReport` CRDs gewährt.  Diese Berechtigungen ermöglichen es dem Scanner, Ressourcen abzurufen, um Auditbewertungen durchzuführen und Richtlinienberichte basierend auf den Bewertungsergebnissen zu erstellen.
