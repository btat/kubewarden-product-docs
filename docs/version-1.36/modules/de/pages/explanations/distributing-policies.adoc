...
:description: Richtlinien sicher über ein OCI-konformes Registry mit Tools wie kwctl verteilen und die Sicherheit des Clusters durch {admc-name} Richtlinienmanagement gewährleisten.
...
{admc-name} Richtlinien sind WebAssembly (Wasm) Binärdateien, die vom {admc-short-name} Policy Server ausgewertet werden.

Der {admc-short-name} Policy Server lädt Richtlinien aus diesen Quellen:
...
Das Ziel-OCI-konforme Registry *muss Artefakte* unterstützen, um erfolgreich eine {admc-short-name} Richtlinie dorthin zu pushen.

Sie können die https://github.com/kubewarden/kwctl[`kwctl`] CLI verwenden, um eine
{admc-short-name} Richtlinie in ein OCI-konformes Registry zu pushen.

== Die Richtlinie annotieren

Sie annotieren auch eine Richtlinie mit `kwctl`. Das Annotieren einer {admc-short-name}
...
* die {admc-short-name} Richtlinie, die annotiert werden soll, eine lokale Datei im Dateisystem.
...
Sie können eine Richtlinie vom {admc-short-name} Policy Server oder `kwctl` als `registry://<oci-registry>/kubewarden-policies/palindromify-policy:v0.0.1` referenzieren.
