...
= Verteilen einer Gatekeeper-Richtlinie mit {admc-name}
...
Richtlinien müssen annotiert werden, damit sie gepusht und schließlich von der {admc-name} `policy-server` in einem Kubernetes-Cluster ausgeführt werden können.
...
annotations:
  ...
  io.kubewarden.policy.author: The {admc-short-name} Authors
