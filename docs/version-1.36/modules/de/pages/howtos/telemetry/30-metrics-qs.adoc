...
:description: Konfigurieren Sie die Metrikberichterstattung auf dem Policy Server mit Prometheus und {admc-name}, das über Helm-Charts für die Datenerfassung aktiviert ist.
...
Sie können jetzt {admc-short-name} auf die empfohlene Weise mit Helm-Charts installieren.
...
Zuerst sollten Sie das Helm-Repository hinzufügen, das {admc-short-name} enthält:
...
Dann installieren Sie die von {admc-short-name} definierten benutzerdefinierten Ressourcenbeschreibungen (CRDs):
...
Jetzt können Sie den Rest des {admc-short-name} Stacks bereitstellen. Das offizielle Helm-Chart erstellt einen PolicyServer mit dem Namen `default`.

Sie sollten das Helm-Chart so konfigurieren, dass Sie Metriken in {admc-short-name} aktiviert haben. Die `kubewarden-values.yaml` Datei sollte die folgenden Inhalte haben:
...
=== Verwendung des {admc-short-name} Grafana-Dashboards

{admc-short-name} hat ein Grafana-Dashboard mit grundlegenden Metriken, das einen Überblick darüber gibt, wie sich {admc-short-name} im Cluster verhält. Dieses Dashboard ist in den GitHub-Releases des {admc-short-name} policy-server-Repositorys als https://github.com/kubewarden/policy-server/releases/latest/download/kubewarden-dashboard.json[JSON-Datei] oder auf der https://grafana.com/grafana/dashboards/15314[Grafana-Website] verfügbar.
...
Das Grafana-Dashboard hat Bereiche, die den Zustand aller von {admc-short-name} verwalteten Richtlinien anzeigen. Es hat auch richtlinienspezifische Panels.
