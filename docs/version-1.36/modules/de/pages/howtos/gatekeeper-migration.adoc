...
= Migration von Gatekeeper-Richtlinien zu {admc-name}
:page-languages: [en, de, es, fr, ja, pt, zh]
:revdate: 2026-05-14
:page-revdate: {revdate}
:description: Konfigurieren Sie eine {admc-name}-Richtlinie, indem Sie eine vorhandene Gatekeeper-Richtlinie in zwei Hauptschritten migrieren: Kompilierung von Rego-Code in WebAssembly.
...
Dieser Leitfaden zeigt Ihnen, wie Sie eine vorhandene Gatekeeper-Richtlinie in eine {admc-name}-Richtlinie umwandeln. Dieser Prozess umfasst zwei Hauptschritte:
. Kompilieren Sie das Rego-Programm in ein WebAssembly (Wasm)-Modul.
. Verteilen Sie das WebAssembly-Modul als {admc-short-name}-Richtlinie.

Das xref:/tutorials/writing-policies/rego/01-intro-rego.adoc[Rego-Richtlinien-Tutorial] behandelt den Großteil des Build-Prozesses zur Kompilierung von Rego-Code in ein WebAssembly-Modul. Dieser Leitfaden konzentriert sich auf den schrittweisen Prozess der Extraktion von Gatekeeper Custom Resource Definitions (CRDs) und deren Migration in eine funktionale {admc-short-name}-Richtlinie. Es verwendet eine grundlegende Gatekeeper-Demo-Richtlinie.
...
* link:https://github.com/kubewarden/kwctl/releases[`kwctl`]: Tool, das Sie verwenden, um das {admc-short-name}-WebAssembly-Modul vorzubereiten und auszuführen.
...
Bevor Sie mit dem Prozess der Migration von Gatekeeper-Richtlinien beginnen, ziehen Sie in Betracht, bereits verfügbare Richtlinien im {admc-short-name} link:https://artifacthub.io/packages/search?kind=13[Katalog] zu verwenden. Einige der link:https://github.com/kubewarden/rego-policies-library[Richtlinien] sind öffentlich verfügbare OPA- und Gatekeeper-Richtlinien, die nach {admc-short-name} migriert wurden.

Sehen Sie sich auch unsere xref:/explanations/comparisons/opa-comparison.adoc[Vergleich] Dokumentation zwischen {admc-short-name} und Gatekeeper an.

== Schritt 1: Initialisieren Sie Ihr {admc-short-name}-Richtlinienprojekt

Zuerst verwenden Sie die {admc-short-name} Gatekeeper
...
`ConstraintTemplate` und ihrer zugehörigen `Constraint` Ressourcen in eine {admc-short-name}
Richtlinie. In einem {admc-short-name} Kontext betrachten Sie die `ConstraintTemplate` als den Kern
Richtliniencode, während die `Constraint` Instanzen in Richtlinieninstanzen
übersetzt werden, die innerhalb von {admc-short-name} laufen.

Zuerst kopieren Sie den Rego-Code von Ihrem `ConstraintTemplate` in die `policy.rego` Datei, die von der {admc-short-name} Vorlage generiert wurde. Für dieses Beispiel sollten Sie die
...
=== Passen Sie den Rego-Code für {admc-short-name} an

Sie müssen sicherstellen, dass der im Rego-Code verwendete `package` Name `policy` ist.
Dies ist der Wert, der an vielen Stellen von der {admc-short-name} Gatekeeper-Vorlage erwartet wird.
...
Die {admc-short-name} Gatekeeper-Vorlage umfasst sowohl Rego-Einheitstests als auch End-to-End (e2e) Tests mit Bats und `kwctl`. Wenn Sie planen, Tests einzuschließen, müssen beide Sätze an Ihre Richtlinie angepasst werden.
...
Diese Richtlinie hat Parameter, die Gatekeeper innerhalb des `Constraint` definiert. Sie müssen den `settings` Abschnitt im generierten {admc-short-name} Richtlinienmanifest aktualisieren, um diese erforderlichen Parameter einzuschließen. Im folgenden Beispiel können Sie, zusätzlich zur Definition der Einstellungen, die Richtlinie aus dem OCI-Registry testen:
