...
:description: Konfigurieren Sie rohe Richtlinien als generische Richtlinienbewertungsmaschine mit {admc-name}.
...
:doc-type: ["howto"]
:keywords: ["{admc-name}", "kubewarden", "kubernetes", "support", "rohe Richtlinien"]
:sidebar_label: Roh-Richtlinien
:sidebar_position: 60
:current-version: {page-origin-branch}

Ab `v1.9.0` unterstützt {admc-name} die Möglichkeit, Richtlinien außerhalb eines Kubernetes-Clusters zu schreiben und auszuführen, als generische Richtlinienbewertungsmaschine.
Der Policy-Server stellt den `/validate_raw` Endpunkt zur Verfügung, der verwendet werden kann, um beliebige JSON-Dokumente gegen eine {admc-short-name} Richtlinie zu validieren.
...
== Ausführen eines Policy-Servers innerhalb von Kubernetes ohne den {admc-short-name} Controller

Es ist nicht möglich, eine vom {admc-short-name} Controller verwaltete Policy-Server-Instanz zu verwenden, um rohe Richtlinien zu hosten.
...
Ähnlich wie bei Richtlinien, die Kubernetes-Ressourcen validieren, werden rohe Richtlinien in WebAssembly unter Verwendung von {admc-short-name} SDKs geschrieben.
