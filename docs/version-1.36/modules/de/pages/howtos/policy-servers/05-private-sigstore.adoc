...
:description: Konfigurieren von PolicyServern zur Verwendung einer privaten oder selbst gehosteten Sigstore-Instanz zur Überprüfung von Richtlinienunterschriften in {admc-name}.
...
Stellen Sie sicher, dass Sie die Zertifikate und Schlüssel sicher von Ihrer Sigstore-Instanz herunterladen. Andernfalls könnten Sie {admc-name} mit kompromittierten Daten konfigurieren.
...
[NOTE]
====
Der ConfigMap-Schlüssel **muss** `verification-config` sein (nicht der Dateiname). {admc-short-name} sucht nach diesem genauen Schlüssel.
====
