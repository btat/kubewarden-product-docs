...
:description: Konfigurieren Sie {admc-name} Richtlinien in Ihren Kubernetes-Clustern, um eine granulare Kontrolle über die Richtlinienkonfiguration zu erreichen und die PodSecurityPolicy (PSP) zu ersetzen.
...
:doc-type: ["howto"]
:keywords: ["{admc-name}", "kubewarden", "kubernetes", "appvia", "psp", "pod security policy"]
...
(PSP) wurde entfernt. Jetzt können Sie {admc-name} für die Zulassungskontrolle in Ihren Kubernetes-Clustern verwenden.

{admc-short-name} hat separate Richtlinien, um dasselbe Ziel wie eine monolithische PSP-Konfiguration zu erreichen. Jede {admc-short-name} Richtliniendefinition fungiert als ein anderer Konfigurationsabschnitt in der Spezifikation einer PSP. Die Zuordnung der PSP-Konfigurationsfelder zu ihren jeweiligen {admc-short-name} Richtlinien befindet sich in der <<_mapping_policies_to_psp_fields,Mapping-Tabelle>> unten.

Mit {admc-short-name} haben Betreiber eine granulare Kontrolle über die Richtlinienkonfiguration in ihren Clustern.

Mit einer {admc-short-name} Instanz können Sie Richtlinien bereitstellen, um das `PodSecurityPolicy` Objekt zu ersetzen. Wir betrachten diese Regeln in diesem Beispiel:
...
== {admc-short-name} Ersetzungen für PSP

Jetzt werden wir {admc-short-name} Richtlinien erstellen, um dasselbe Ziel zu erreichen.
Sie setzen jede Regel mit einer separaten {admc-short-name} Richtlinie durch.
...
== Zuordnung von {admc-short-name} Richtlinien zu PSP-Feldern

Diese Tabelle ordnet die PSP-Konfigurationsfelder den entsprechenden {admc-short-name} Richtlinien zu.

|===
| PSP-Feld | {admc-short-name} äquivalente Richtlinie
...
Das {admc-short-name} Team hat ein Skript für die PSP-Migration entwickelt. Es verwendet das Migrationswerkzeug von https://github.com/appvia/psp-migration[AppVia]. Das AppVia-Werkzeug liest eine PSP-YAML-Konfiguration. Anschließend generiert es die entsprechenden Richtlinien. Dies geschieht für {admc-short-name} und andere Richtlinien-Engines.

[CAUTION]
====

Das AppVia-Migrationswerkzeug liegt außerhalb der Kontrolle der Kuberwarden-Maintainer.
Das bedeutet, dass es möglich ist, dass es veraltete {admc-short-name} Richtlinien generiert. Mit Vorsicht verwenden. Wir benötigen einen Pull-Request für AppVia, an dem derzeit gearbeitet wird.
Kontaktieren Sie uns für weitere Informationen, falls Sie dies benötigen.
====


Das Skript ist im {admc-short-name} https://github.com/kubewarden/utils/blob/main/scripts/psp-to-kubewarden[utils] Repository verfügbar. Es lädt das AppVia-Migrationswerkzeug in das Arbeitsverzeichnis herunter, um es zu verwenden. Es verarbeitet die in dem `kubectl` Standardkontext definierten PSPs. Dann gibt es die Kuberwarden-Richtliniendefinitionen auf der Standardausgabe aus. Benutzer können den Inhalt entweder in eine Datei oder direkt nach `kubectl` umleiten.
...
{admc-short-name} Richtlinien können direkt auf einen Cluster mit dem folgenden Befehl angewendet werden:
