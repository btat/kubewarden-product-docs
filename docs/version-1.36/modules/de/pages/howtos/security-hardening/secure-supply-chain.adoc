...
:description: Konfigurieren Sie {admc-name}, um die Integrität von Artefaktsignaturen und Richtlinien zu überprüfen.
...
:doc-type: ["howto"]
:doctype: book
:keywords: ["{admc-name}", "kubewarden", "kubernetes", "sichere Lieferkette", "infrastruktur"]
...
{admc-short-name} verwendet https://github.com/sigstore/cosign[`cosign`] zusammen mit der https://github.com/SigStore/fulcio[fulcio] und https://github.com/sigstore/rekor[rekor] Infrastruktur, die vom Sigstore-Projekt angeboten wird.

Clusterbetreiber können {admc-name} so konfigurieren, dass nur Richtlinien ausgeführt werden, die von vertrauenswürdigen Stellen signiert sind. Richtlinienentwickler können ihre Richtlinien signieren und in einem Register veröffentlichen.
...
Dieser Befehl signiert die Richtlinie, indem er ein neues Signaturobjekt erstellt. Das Signaturobjekt wird dann zusammen mit der Richtlinie in das Registry hochgeladen. Jetzt ist die Richtlinie bereit zur Verwendung in einer {admc-short-name}-Installation mit Signaturüberprüfung.
...
Richtlinienentwickler können die {admc-short-name} Richtlinienvorlagen verwenden. Sie haben GitHub-Aktionen, um Richtlinien zu erstellen, zu testen, zu signieren und zu veröffentlichen.
...
Mit {admc-short-name} können Sie überprüfen, ob die Richtliniensignatur von vertrauenswürdigen Benutzern *und* stammt, die spezifische Annotationen haben.
...
Dieses letzte Beispiel testet, ob eine gegebene Richtlinie von der {admc-short-name} Organisation stammt:
