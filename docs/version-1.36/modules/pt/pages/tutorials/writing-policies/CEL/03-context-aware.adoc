...
:description: Configure políticas com reconhecimento de contexto para garantir regras de Ingress únicas entre hosts, evitando conflitos com as bibliotecas de extensão CEL de {admc-name}.
...
O {admc-name} de `cel-policy` suporta o recurso de xref:reference/spec/05-context-aware-policies.adoc[consciência de contexto]. A política tem a capacidade de ler informações do cluster e tomar decisões com base em outros recursos existentes além daquele que acionou a avaliação da política via solicitação de admissão.

Para alcançar isso, podemos usar as bibliotecas de extensão CEL de https://github.com/kubewarden/cel-policy?tab=readme-ov-file#host-capabilities[{admc-short-name} para capacidades de host] incluídas na política.
...
Agora, precisamos escrever o código CEL que buscará os Ingresses existentes no cluster. Para isso, usamos a https://github.com/kubewarden/cel-policy?tab=readme-ov-file#host-capabilities[biblioteca de extensão CEL {admc-short-name}].
Particularmente, as `kw.k8s` capacidades de host, que nos permitem consultar o cluster por GroupVersionKinds. Você pode ver a documentação disponível para as funções CEL https://pkg.go.dev/github.com/kubewarden/cel-policy/internal/cel/library[aqui].
