...
Como operador de cluster Kubernetes, você vai querer realizar testes para as políticas {admc-name} que deseja usar.
...
{admc-short-name} possui uma utilidade, https://github.com/kubewarden/kwctl[`kwctl`], que permite testar as políticas fora do Kubernetes.

Para usar `kwctl`, você a invoca com as seguintes entradas:

. Um URI de arquivo binário WebAssembly da política a ser executada.
A política {admc-short-name} pode carregar do:
