...
:description: Distribua políticas de forma segura por meio de um registro compatível com OCI usando ferramentas como kwctl, e garanta a segurança do cluster por meio do gerenciamento de políticas {admc-name}.
...
:keywords: ["{admc-name}", "oci", "kubewarden", "política", "wasm", "webassembly"]
:current-version: {page-origin-branch}

As políticas {admc-name} são binários WebAssembly (Wasm) avaliados pelo servidor de políticas {admc-short-name}.

O servidor de políticas {admc-short-name} carrega políticas dessas fontes:
...
O registro compatível com OCI de destino *deve suportar artefatos* para enviar com sucesso uma {admc-short-name} política para ele.

Você pode usar o CLI https://github.com/kubewarden/kwctl[`kwctl`] para enviar uma {admc-short-name} política para um registro compatível com OCI.

== Anotando a política

Você também anota uma política com `kwctl`. Anotar uma {admc-short-name}política é feito adicionando seções personalizadas Wasm ao binário da política. Isso significa que a política empacota seus próprios metadados.

O comando `kwctl annotate` requer duas entradas:

* a {admc-short-name}política a ser anotada, um arquivo local no sistema de arquivos.
...
Você pode referenciar uma política a partir do {admc-short-name}servidor de políticas ou de `kwctl` como
