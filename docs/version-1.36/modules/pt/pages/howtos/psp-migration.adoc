...
:description: Configure {admc-name} políticas em seus clusters Kubernetes para obter controle granular sobre a configuração de políticas, substituindo PodSecurityPolicy (PSP).
...
:doc-type: ["howto"]
:keywords: ["{admc-name}", "kubewarden", "kubernetes", "appvia", "psp", "política de segurança de pod"]
...
https://kubernetes.io/docs/concepts/security/pod-security-policy/[PodSecurityPolicy] (PSP) foi removido. Agora você pode usar {admc-name} para controle de admissão em seus clusters Kubernetes.

{admc-short-name} possui políticas separadas para alcançar o mesmo objetivo que uma configuração monolítica de PSP. Cada definição de política {admc-short-name} funciona como uma seção de configuração diferente na especificação de um PSP. O mapeamento dos campos de configuração do PSP para suas respectivas políticas {admc-short-name} está na <<_mapping_policies_to_psp_fields,tabela de mapeamento>> abaixo.

Com {admc-short-name}, os operadores têm controle granular da configuração de políticas em seus clusters.

Com uma instância de {admc-short-name}, você pode implantar políticas para substituir o objeto `PodSecurityPolicy`. Consideramos essas regras neste exemplo:
...
== Substituições de {admc-short-name} para PSP

Agora criaremos {admc-short-name} políticas para alcançar o mesmo objetivo.
Você aplica cada regra com uma política {admc-short-name} separada.
...
== Mapeando políticas {admc-short-name} para campos PSP

Esta tabela mapeia os campos de configuração do PSP para as políticas {admc-short-name} correspondentes.

|===
| Campo PSP | Política equivalente a {admc-short-name}
...
A equipe {admc-short-name} desenvolveu um script para migração do PSP. Ele utiliza a ferramenta de migração do https://github.com/appvia/psp-migration[AppVia]. A ferramenta AppVia lê uma configuração YAML do PSP. Em seguida, gera as políticas correspondentes. Faz isso para {admc-short-name} e outros mecanismos de política.

[CAUTION]
====

A ferramenta de migração AppVia está fora do controle dos mantenedores do Kubewarden.
Isso significa que é possível que ela gere políticas {admc-short-name} desatualizadas. Use com cautela. Precisamos de um pull request para o AppVia, para o qual o trabalho está em andamento.
Entre em contato conosco para mais informações, se necessário.
====


O script está disponível no repositório {admc-short-name} https://github.com/kubewarden/utils/blob/main/scripts/psp-to-kubewarden[utils]. Ele baixa a ferramenta de migração AppVia para o diretório de trabalho para uso. Ele processa os PSPs definidos no contexto padrão `kubectl`. Em seguida, imprime as definições das políticas do Kubewarden na saída padrão. Os usuários podem redirecionar o conteúdo para um arquivo ou para `kubectl` diretamente.
...
{admc-short-name} políticas podem ser aplicadas diretamente a um cluster usando o seguinte comando:
