...
Por padrão, o Admission Controller não permite que políticas avaliem recursos em seu próprio namespace. Isso previne configurações incorretas que podem quebrar a {admc-short-name} instalação.
...
Para fazer isso, atualize sua {admc-short-name} instalação e defina o `alwaysAcceptAdmissionReviewsOnDeploymentsNamespace` valor do `kubewarden-controller` chart Helm para `false`:
...
Uma vez feito isso, as políticas podem ser configuradas para avaliar revisões de admissão do {admc-short-name} namespace.
...
====
Os CRDs de políticas também permitem que os usuários definam seus próprios seletores de namespace. Esses seletores não são alterados. Isso significa que, se um usuário definir um seletor que ignora o {admc-short-name} namespace, a política ainda ignorará esse namespace como esperado.
====