...
:description: Configurando PolicyServers para usar uma instância privada ou auto-hospedada do Sigstore para verificação de assinatura de políticas em {admc-name}.
...
Certifique-se de baixar os certificados e chaves com segurança de sua instância Sigstore. Caso contrário, você pode acabar configurando {admc-name} com dados comprometidos.
...
[NOTE]
====
A chave do ConfigMap **deve** ser `verification-config` (não o nome do arquivo). {admc-short-name} procura por esta chave exata.
====
