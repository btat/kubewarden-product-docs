...
:keywords: "{admc-name}", Kubernetes, Kubewarden, Operador de Backup do Rancher, backup, restore
...
Desde a versão `v9.0.0`, `rancher-backup` tem suporte para {admc-name}. Isso inclui:

- O Namespace padrão do Rancher `cattle-kubewarden-system` (ou `cattle-kubewarden-*`), e o Namespace padrão {admc-short-name} `kubewarden`.
- Os recursos necessários de {admc-short-name} são instalados por meio dos charts Helm.
- Os CRDs de {admc-short-name}, que são reconciliados após a restauração pelo controlador {admc-short-name}.
...
É necessário instruir o Operador de Backup a também fazer backup de CRs definidos pelo usuário {admc-short-name} definindo o Valor `optionalResources.kubewarden.enabled` como `true`.
...
Use o `rancher-resource-set-full` para fazer backup dos segredos {admc-short-name}.
