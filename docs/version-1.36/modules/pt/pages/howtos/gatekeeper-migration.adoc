...
= Migrando Políticas do Gatekeeper para {admc-name}
:page-languages: [en, de, es, fr, ja, pt, zh]
:revdate: 2026-05-14
:page-revdate: {revdate}
:description: Configure uma política {admc-name} migrando uma política existente do Gatekeeper através de duas etapas principais: compilando o código Rego em WebAssembly.
...
Este guia mostra como converter uma política existente do Gatekeeper em uma política {admc-name}. Este processo envolve duas etapas principais:
. Compile o programa Rego em um módulo WebAssembly (Wasm).
. Distribua o módulo WebAssembly como uma política {admc-short-name}.

O tutorial de políticas xref:/tutorials/writing-policies/rego/01-intro-rego.adoc[Rego] cobre a maior parte do processo de construção para compilar o código Rego em um módulo WebAssembly. Este guia foca no processo passo a passo de extrair as Definições de Recursos Personalizados (CRDs) do Gatekeeper e migrá-las para uma política {admc-short-name} funcional. Ele utiliza uma política de demonstração básica do Gatekeeper.
...
* link:https://github.com/kubewarden/kwctl/releases[`kwctl`]: ferramenta que você usa para preparar e executar o módulo de WebAssembly {admc-short-name}.
...
Antes de iniciar o processo de migração das políticas do Gatekeeper, considere usar políticas já disponíveis no {admc-short-name} link:https://artifacthub.io/packages/search?kind=13[catalog]. Algumas das link:https://github.com/kubewarden/rego-policies-library[políticas] são políticas OPA e Gatekeeper publicamente disponíveis migradas para {admc-short-name}.

Além disso, dê uma olhada na nossa documentação de xref:/explanations/comparisons/opa-comparison.adoc[comparação] entre {admc-short-name} e Gatekeeper.

== Etapa 1: Inicialize seu projeto de política {admc-short-name}

Primeiro, use o Gatekeeper {admc-short-name}
...
`ConstraintTemplate` e seus recursos associados `Constraint` em uma política {admc-short-name}. Em um contexto {admc-short-name}, considere o `ConstraintTemplate` como o código kernel da política, enquanto as instâncias `Constraint` se traduzem em instâncias de política executando dentro de {admc-short-name}.

Primeiro, copie o código Rego do seu `ConstraintTemplate` para o arquivo `policy.rego` que foi gerado pelo template {admc-short-name}. Para este exemplo, você deve usar a seguinte política de demonstração link:https://github.com/open-policy-agent/gatekeeper/blob/896d6620f9c16d7a5d91a74a6a4260db8d735640/demo/basic/demo.sh#L1[demo policy] do repositório do Gatekeeper.
...
=== Adaptar o código Rego para {admc-short-name}

Você precisa garantir que o nome `package` usado dentro do código Rego seja `policy`.
Este é o valor esperado em muitos lugares pelo modelo do Gatekeeper {admc-short-name}.
...
O template {admc-short-name} do Gatekeeper inclui tanto testes unitários de Rego quanto testes de ponta a ponta (e2e) usando Bats e `kwctl`. Se você planeja incluir testes, ambos os conjuntos precisam ser adaptados para sua política.
