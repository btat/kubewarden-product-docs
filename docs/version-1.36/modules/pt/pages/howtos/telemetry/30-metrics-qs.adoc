...
:description: Configure a coleta de métricas no Policy Server usando Prometheus e {admc-name} habilitado através de Helm charts para coleta de dados.
...
Agora você pode instalar {admc-short-name}, da maneira recomendada com Helm
charts.
...
Primeiro, você deve adicionar o repositório Helm que contém {admc-short-name}:
...
Em seguida, você instala as Definições de Recursos Personalizados (CRDs) definidas pelo {admc-short-name}:
...
Agora você pode implantar o restante da pilha do {admc-short-name}. O Helm chart oficial cria um PolicyServer chamado `default`.

Você deve configurar o Helm chart para que as métricas estejam habilitadas em {admc-short-name}. O arquivo `kubewarden-values.yaml` deve ter o seguinte conteúdo:
...
=== Usando o {admc-short-name} painel do Grafana

{admc-short-name} tem um painel do Grafana com métricas básicas que fornecem uma visão geral de como {admc-short-name} se comporta no cluster. Este painel está disponível nas versões do GitHub do repositório policy-server {admc-short-name} como um https://github.com/kubewarden/policy-server/releases/latest/download/kubewarden-dashboard.json[arquivo JSON] ou no https://grafana.com/grafana/dashboards/15314[site do Grafana].
...
O painel do Grafana possui painéis mostrando o estado de todas as políticas gerenciadas por {admc-short-name}. Ele também possui painéis específicos para políticas.
