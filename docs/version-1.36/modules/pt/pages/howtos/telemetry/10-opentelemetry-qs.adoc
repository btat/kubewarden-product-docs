...
Os componentes de {admc-name}, usando o SDK OpenTelemetry, relatam dados a um coletor OpenTelemetry — chamado de agente.
...
* Cada Pod da pilha {admc-short-name} (PolicyServer, Controlador) possui um sidecar OpenTelemetry.
* O sidecar recebe informações de rastreamento e monitoramento do componente {admc-short-name} via o Protocolo OpenTelemetry (OTLP)
...
O gráfico Helm {admc-short-name} não cobre todos os possíveis cenários de implantação do coletor OpenTelemetry. Também é possível configurar {admc-short-name} para enviar dados a um coletor OpenTelemetry. A documentação para esse cenário está no xref:./40-custom-otel-collector.adoc[guia OpenTelemetry personalizado].
