...
:description: Configure {admc-name} para validar a integridade das assinaturas de artefatos e políticas de admissão.
...
:doc-type: ["howto"]
:doctype: book
:keywords: ["{admc-name}", "kubewarden", "kubernetes", "cadeia de suprimento segura", "infraestrutura"]
...
{admc-short-name} usa https://github.com/sigstore/cosign[`cosign`] juntamente com a infraestrutura https://github.com/SigStore/fulcio[fulcio] e https://github.com/sigstore/rekor[rekor] oferecida pelo projeto Sigstore.

Operadores de cluster podem configurar {admc-name} para executar apenas políticas assinadas por entidades confiáveis. Desenvolvedores de políticas podem assinar suas políticas e publicá-las em um registro.
...
Este comando assina a política criando um novo objeto de assinatura. O objeto de assinatura é então enviado para o registro, junto com a política. Agora a política está pronta para ser usada em uma instalação {admc-short-name} usando verificação de assinatura.
...
Os desenvolvedores de políticas podem usar os modelos de política {admc-short-name}. Eles têm ações do GitHub para construir, testar, assinar e publicar políticas.
...
Com {admc-short-name}, você pode verificar se a assinatura da política é de usuários confiáveis *e* que possuem anotações específicas.
...
Este último exemplo testa se uma política dada veio da organização {admc-short-name}:
